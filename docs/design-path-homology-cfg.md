...
| 2 | **Binary CFG extraction.** Per-function basic-block CFG via r2 `afbj`/`agfj` in `radare2_understand.py`; `Graph` adapter; per-build-id cache. | Extraction + adapter + cache; CFGs available on `FunctionInfo`. No scoring change. | not started |
| 3 | **Compute + surface (reported only).** Betti vector per prioritized function; attach to `FunctionInfo`, `fuzz_priorities`, `context-map.json`; derive `decompiler_low_confidence`. All behind `--path-homology` (off by default). | Reported fields + decompiler-confidence tag; no effect on ranking yet. | not started |
