...
| `GLASGOW` | unset | Explicit path to the `glasgow` binary for the hardware applet harness (`packages/hardware/scripts/glasgow_applet_harness.py`). Env > `PATH` lookup > known install locations. Set it when glasgow lives in a source-install venv. |
