...
class TestReachabilityGate:
    def test_entry_point_never_demoted(self):
        outcome = _outcome()
        ctx = _ctx(role="entry_point")
        result = _apply_reachability_gate(
            outcome, ctx, {"a.c:foo"}, _config(),
        )
        assert result.status == "finding"

    def test_sink_never_demoted(self):
        outcome = _outcome()
        ctx = _ctx(role="sink", dangerous_apis=["strcpy"])
        result = _apply_reachability_gate(
            outcome, ctx, set(), _config(),
        )
        assert result.status == "finding"

    def test_no_callers_not_entry_demoted_to_suspicious(self):
        outcome = _outcome()
    ...
    def test_no_callers_but_entry_point_kept(self):
        outcome = _outcome()
        ctx = _ctx(role="internal", callers=[])
        result = _apply_reachability_gate(
            outcome, ctx, {"a.c:foo"}, _config(),
    ...
    def test_on_flow_path_kept(self):
        outcome = _outcome()
    ...
    def test_leaf_no_dangerous_apis_kept(self):
    ...
    def test_internal_with_callers_kept(self):
        outcome = _outcome()
        ctx = _ctx(
            role="internal",
            callers=[{"file": "b.c", "name": "bar", "line_start": 1}],
    ...
    def test_binary_absent_demoted_to_dormant(self):
        outcome = _outcome()
        ctx = _ctx(role="internal", callers=[])
        config = _config(binary_verdicts={"foo": "absent"})
        result = _apply_reachability_gate(
    ...
    def test_binary_present_not_demoted(self):
        outcome = _outcome()
        ctx = _ctx(
            role="internal",
        ...
        config = _config(binary_verdicts={"foo": "symbol_present"})
        result = _apply_reachability_gate(
            outcome, ctx, set(), config,
        )
        assert result.status == "finding"

    def test_cost_preserved_through_demotion(self):
        outcome = _outcome()
    ...
    def test_no_caller_data_skips_demotion(self):
    ...
    def test_clean_status_not_gated(self):
