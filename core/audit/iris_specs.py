...
def compile_codeql_config(
    specs: list[TaintSpec], *, language: str = "cpp",
) -> str:
    """Generate a runnable CodeQL query from specs.

    Delegates to ``core.iris.specs.compile_codeql_config`` — this twin
    used to duplicate the generation with a diverged body whose
    ``DataFlow::CallNode`` matching (and class-based
    ``TaintTracking::AdditionalTaintStep``) exists in no language's
    current QL library, so its output could never compile. One
    generator, one set of compile-verified per-language templates; the
    two TaintSpec shapes are attribute-compatible across the seam (see
    the dataclass docstring), converted explicitly for type integrity.

    ``language`` must be in ``core.iris.specs.CODEQL_QUERY_LANGUAGES``;
    anything else raises ``ValueError`` (fail-closed, matching the
    canonical generator).
    """
    from core.iris.specs import TaintSpec as _IrisTaintSpec
    from core.iris.specs import compile_codeql_config as _compile

    converted = [
        _IrisTaintSpec(function=s.function, file=s.file, role=s.role)
        for s in specs
    ]
    return _compile(converted, language=language)
