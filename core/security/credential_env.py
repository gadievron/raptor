...
BUILD_ECOSYSTEM_ENV_SURFACES: tuple[BuildEcosystemEnvSurface, ...] = (
    ...
    BuildEcosystemEnvSurface(
        ...
        completeness=(
            ...
            "instead of escaping it. Named boundary of that oracle: "
            "its observation window is NON-EXECUTING by design "
            "(no-makefile startup plus two `-n` dry runs, inert "
            "payloads), so a name that expands ONLY at real recipe "
            "execution can never light up in it, on any make version "
            "— GNU Make 4.3's MAKEOVERRIDES is the concrete instance "
            "(expanded when a recipe actually executes, invisible to "
            "the probes; already belted, so no live gap). Belts for "
            "the exec-only-expansion class come from documentation/"
            "CVE-style derivation, never from the sweep. meson "
