"""Docker-stderr failure classifier.

A vague "docker failed" masks the real cause — e.g. Colima VM disk
exhaustion mid-pull (`no space left on device`). Without a categorical
signal a caller cannot distinguish "host can't store this right now"
from "image truly absent on the registry."
...
Purely textual — no subprocess, no I/O. Callers (the container run/
build/compose surfaces here and the cve-env planner) populate a
``reason_class`` field on their result payloads from it.
"""
