...
- **Rust, Ruby, C#, PHP, C, C++, Lua, Scala, Kotlin, Swift** — the same tree-sitter pattern (fourteen `extract_call_graph_*` extractors ship in total, each gracefully empty when its grammar isn't installed). The four entries above document the walkers with resolver-specific import/indirection semantics; the rest emit the shared `FileCallGraph` shape.
