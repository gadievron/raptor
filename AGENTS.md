...
**Skills** (in `.agents/skills/crash-analysis/`):
...
**Skills** (in `.agents/skills/oss-forensics/`):
...
**Stages:** 0 → A → B → C → D → E → F → 1 (see `.agents/skills/exploitability-validation/PIPELINE.md`)

**Skills** (in `.agents/skills/exploitability-validation/`):
...
**Skills** (in `.agents/skills/code-understanding/`):
...
**When validating exploitability:** Load `.agents/skills/exploitability-validation/SKILL.md` (gates, methodology)
...
**When running /understand:** Load `.agents/skills/code-understanding/SKILL.md` (gates, config) plus the relevant mode file: `map.md`, `trace.md`, `hunt.md`, or `teach.md`
...
Python orchestrates everything. Codex shows results concisely.
