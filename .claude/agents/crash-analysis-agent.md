...
When SAGE is available, use persistent memory to learn across sessions:
- **Boot**: Call `sage_inception` at session start
- **Every turn**: Call `sage_turn` with topic and observation, domain: `raptor-crashes`
- **After analysis**: Store crash patterns and root causes via `sage_remember` in domain `raptor-crashes`
- **Before starting**: Call `sage_recall` with domain `raptor-crashes` to check for known crash patterns in similar code
- **After completion**: Call `sage_reflect` with dos/don'ts learned
