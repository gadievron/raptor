...
**You (Codex) ARE the LLM for this pipeline.** You perform the analysis work in LLM stages (A-D, F). Mechanical stages (0, E, 1) run via Python/libexec scripts.

**Data flow:** Each stage writes a small `stage-X.json` file with only its own output. The prep script merges it into the cumulative `findings.json` and deletes the stage file. Codex never reads or writes findings.json directly.
...
### Stage A (Codex): One-Shot Assessment

**Load:** `.agents/skills/exploitability-validation/stage-a-oneshot.md`
...
### Stage B (Codex): Systematic Analysis

**Load:** `.agents/skills/exploitability-validation/stage-b-process.md`
...
### Stage C (Codex): Sanity Check

**Load:** `.agents/skills/exploitability-validation/stage-c-sanity.md`
...
### Stage D (Codex): Ruling

**Load:** `.agents/skills/exploitability-validation/stage-d-ruling.md`
...
### Stage E (Codex + Python): Feasibility — memory corruption only

**Load:** `.agents/skills/exploitability-validation/stage-e-feasibility.md`
...
### Stage F (Codex): Self-Review

**Load:** `.agents/skills/exploitability-validation/stage-f-review.md`
...
This command enforces strict validation gates. Full definitions are in `.agents/skills/exploitability-validation/SKILL.md` — read it before starting.
