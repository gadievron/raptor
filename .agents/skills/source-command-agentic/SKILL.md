...
Use this skill when the user asks to run the migrated source command `agentic`.
...
- **Codex on PATH**: dispatches `codex -p` sub-agents (separate processes)
- **External LLM configured**: dispatches via `generate_structured()` API calls
- **Both available**: uses external LLM, falls back to Codex if it fails
...
**YOU (Codex) are the LLM** — the user may ask you to analyse the findings
...
`--sequential` was used or Codex was not available). Present the results.

**`"mode": "orchestrated"`** — Parallel analysis via external LLM or Codex
