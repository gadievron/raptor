...
**Binary targets:** If `<resolved_target>` is an executable binary and the requested mode is `--map`, use the shared radare2 binary analyser instead of source inventory:
```bash
libexec/raptor-binary-understand <resolved_target> "$OUTPUT_DIR"
libexec/raptor-render-diagrams "$OUTPUT_DIR" || true
libexec/raptor-run-lifecycle complete "$OUTPUT_DIR"
```
If this fails, mark the lifecycle failed and report the radare2/r2pipe error. For source directories, continue with the normal checklist flow below.
...
- `.agents/skills/code-understanding/SKILL.md` — gates, config, output format
- `.agents/skills/code-understanding/map.md` — for `--map`
- `.agents/skills/code-understanding/trace.md` — for `--trace`
- `.agents/skills/code-understanding/hunt.md` — for `--hunt`
- `.agents/skills/code-understanding/teach.md` — for `--teach`
