...
def validate_cve_id(cve_id: str) -> str:
    ...
    if cve_id is None:
        msg = "CVE ID cannot be None, must be string"
        raise ValidationError(msg)
    if not isinstance(cve_id, str):
        msg = f"CVE ID must be string, not {type(cve_id).__name__}"
        raise ValidationError(msg)
    if not cve_id or not cve_id.strip():
        msg = "CVE ID cannot be empty"
        raise ValidationError(msg)
    if cve_id != cve_id.strip():
        msg = "CVE ID cannot contain leading/trailing whitespace"
        raise ValidationError(msg)

    for token in _SQL_INJECT_TOKENS:
        if token in cve_id:
            msg = "CVE ID contains invalid characters (possible SQL injection attempt)"
            raise ValidationError(msg)

    if ".." in cve_id or "/" in cve_id or "\\" in cve_id:
        msg = "CVE ID contains invalid characters (possible path traversal attempt)"
        raise ValidationError(msg)
    ...
    if not match:
        if not cve_id.startswith("CVE-"):
            if cve_id.lower().startswith("cve-"):
                msg = "CVE ID must use uppercase 'CVE-', not lowercase"
                raise ValidationError(msg)
            msg = "CVE ID must start with 'CVE-' prefix (uppercase)"
            raise ValidationError(msg)
        if cve_id.count("-") > 2:
            msg = "CVE ID format invalid (too many hyphens)"
            raise ValidationError(msg)
        parts = cve_id.split("-")
        if len(parts) >= 2:
            year_part = parts[1]
            if not year_part.isdigit():
                msg = "CVE ID year must be numeric (YYYY format)"
                raise ValidationError(msg)
            if len(year_part) != 4:
                msg = "CVE ID year must be 4 digits (YYYY format)"
                raise ValidationError(msg)
        if len(parts) >= 3:
            id_part = parts[2]
            if not id_part.isdigit():
                msg = "CVE ID number must be numeric (no letters)"
                raise ValidationError(msg)
            if len(id_part) < 4:
                msg = "CVE ID number must be at least 4 digits"
                raise ValidationError(msg)
        msg = "CVE ID format invalid (expected: CVE-YYYY-NNNN)"
        raise ValidationError(msg)

    year_str, id_str = match.group(1), match.group(2)
    if not year_str.isascii() or not id_str.isascii():
        msg = "CVE ID must contain only ASCII characters (no unicode)"
        raise ValidationError(msg)

    year = int(year_str)
    current_year = datetime.now().year
    if year < 1999:
        msg = "CVE ID year must be 1999 or later (CVE program started in 1999)"
        raise ValidationError(msg)
    if year > current_year + 1:
        msg = f"CVE ID year cannot be in distant future (max: {current_year + 1})"
        raise ValidationError(msg)
    if len(id_str) > 10:
        msg = "CVE ID number too long (max 10 digits)"
        raise ValidationError(msg)
