...
from core.atomic_fs import write_new_text, write_text_atomically
...
def _write_generated_source(
    ...
    *,
    ...
    # Exclusive create (lstat-honest replace): the harness dir is
    # reused run output — a planted symlink at these predictable
    # names would send generated source / an EXECUTABLE build script
    # to an attacker-chosen path.
    write_new_text(source_path, source, replace=True)
...
def generate_binary_harness(
    ...
    *,
    ...
    # Atomic like the save_json below (planted-symlink defence in the
    # reused harness dir).
    write_text_atomically(report_path, render_harness_report(spec))
    spec["artifacts"] = {
        "spec": str(spec_path),
