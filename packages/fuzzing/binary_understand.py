...
drives radare2 via r2pipe, extracts the structural information a fuzzer
needs, optionally decompiles high-value functions through r2ghidra, and
asks the LLM to identify entry points, trust boundaries, and dangerous
sinks based on the decompiled output.

Output is a BinaryContextMap with the same shape as the source-level
context-map.json so downstream consumers (harness generator, fuzzing
orchestrator) can treat both kinds of analysis uniformly.
...
class BinaryContextMap:
    ...
    def to_dict(self) -> Dict[str, Any]:
        def fn_dict(f: FunctionInfo) -> Dict[str, Any]:
        ...
        return {
            ...
            "entry_points": [fn_dict(f) for f in self.entry_points],
            "dangerous_sinks": [fn_dict(f) for f in self.dangerous_sinks],
