...
class HarnessGenerator:
    ...
    def write(self, harness: GeneratedHarness, out_dir: Path) -> Path:
        ...
        # Exclusive create (lstat-honest replace): the out dir is
        # reused run output — a planted symlink at these predictable
        # names would send generated source / an EXECUTABLE build
        # script to an attacker-chosen path.
        write_new_text(target_path, harness.source_code, replace=True)
