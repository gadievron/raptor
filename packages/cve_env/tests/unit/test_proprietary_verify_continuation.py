...
def test_gate_on_by_default(monkeypatch: Any) -> None:
    ...
    assert _should_continue_for_proprietary_verify(_run_stub(), st2, 0, 0.1, 2.5) is False
...
def test_gate_skips_already_probed_proprietary(_on: None) -> None:
    ...
    assert _should_continue_for_proprietary_verify(_run_stub(), st, 0, 0.1, 2.5) is False
...
def test_gate_skips_non_proprietary(_on: None) -> None:
    ...
    for reason in ("no_image", "arch_incompatible", "skipped_image_lookup", "budget"):
        ...
        assert _should_continue_for_proprietary_verify(_run_stub(), st, 0, 0.1, 2.5) is False, reason
...
def test_gate_requires_resumable_session(_on: None) -> None:
    ...
    from cve_env.agent.loop import _should_continue_for_proprietary_verify
    st = _state("proprietary", ["nvd_lookup", "give_up"])
    assert _should_continue_for_proprietary_verify(_run_stub(session_id=""), st, 0, 0.1, 2.5) is False
...
def test_gate_respects_max(_on: None) -> None:
    ...
    from cve_env.agent.loop import _should_continue_for_proprietary_verify
    st = _state("proprietary", ["nvd_lookup", "give_up"])
    assert _should_continue_for_proprietary_verify(_run_stub(), st, 1, 0.1, 2.5) is False
...
def test_gate_respects_budget_fraction(_on: None) -> None:
    ...
    assert _should_continue_for_proprietary_verify(_run_stub(), st, 0, 2.0, 2.5) is False
...
@pytest.mark.parametrize("tools,expect_fire", [
    (["nvd_lookup", "give_up"], True),                         # Cisco/SAP/Oracle no-probe
    (["nvd_lookup", "github_fetch", "give_up"], True),         # found PoC repo, no image probe
    (["nvd_lookup", "image_resolve", "give_up"], False),       # Zimbra-class: probed, negative
    (["nvd_lookup", "image_resolve", "github_fetch", "give_up"], False),  # probed + searched
])
def test_known_proprietary_classes(_on: None, tools: list[str], expect_fire: bool) -> None:
    from cve_env.agent.loop import _should_continue_for_proprietary_verify
    st = _state("proprietary", tools)
    assert _should_continue_for_proprietary_verify(_run_stub(), st, 0, 0.1, 2.5) is expect_fire
...
def test_audit_status_registers_all_continuation_statuses() -> None:
    ...
    for status in (
        ...
        assert status in registered, f"{status} emitted but not registered in AuditStatus"
