...
# ---------------------------------------------------------------------------
# Markdown inertness — remote/repo strings in the PR comment
# ---------------------------------------------------------------------------
...
_PAYLOAD = f"{_IMG} {_LINK} <img src=https://evil.example/h> | `x` |"
...
def _hostile_report(tmp_path: Path) -> BumpReport:
    dep = Dependency(
        ecosystem="Helm", name=_PAYLOAD, version="1.0.0",
        declared_in=tmp_path / "Chart.yaml", scope="main",
        is_lockfile=False, pin_style=PinStyle.EXACT, direct=True,
        purl=None,
        parser_confidence=Confidence("high", reason="t"),
    )
    sf = SupplyChainFinding(
        finding_id="sca:x", kind="recent_publish", dependency=dep,
        detail=_PAYLOAD + "\x1b[31mANSI\x1b[0m",
        evidence={}, severity="high",
        confidence=Confidence("high", reason="t"),
    )
    vf = VulnFinding(
        finding_id="sca:v", dependency=dep,
        advisories=[_adv(_PAYLOAD)], in_kev=True, epss=None,
        fixed_version=None,
        ...
        cvss_score=9.8, cvss_vector="CVSS:3.1/AV:N", severity="critical",
        exposure_factor=0.0, transitive_depth=0,
    )
    cand = BumpCandidate(
        kind="helm_dep", locator=_PAYLOAD, file=tmp_path / "Chart.yaml",
        current_version="1.0.0", target_version="2.0.0",
    )
    result = BumpResult(
        candidate=cand, verdict=2, verdict_label="Block",
        bump_supply_chain_findings=[sf], bump_vuln_findings=[vf],
        error=_PAYLOAD,
    )
    return BumpReport(
        target=tmp_path, candidates=[cand], results=[result],
        skipped=[(_PAYLOAD, tmp_path / "Chart.yaml",
                  _PAYLOAD + "\nline2 \x1b[2Jclear")],
    )


def test_pr_comment_untrusted_fields_are_inert(tmp_path: Path) -> None:
    """Beacon payload in every remote/repo field of the PR comment —
    locator, advisory id, error text, skip reason — must render inert
    (no live image/link/HTML, no row breakout)."""
    md = render_pr_comment(_hostile_report(tmp_path))
    _assert_inert(md)
    # Table rows must not be split by payload pipes: every table line
    # keeps exactly the 6-column shape (7 '|' separators are the
    # minimum; a raw payload pipe would ADD cells to the row).
    table_rows = [
        ln for ln in md.splitlines()
        if ln.startswith("| ") and "beacon" in ln
    ]
    assert table_rows, md
    for row in table_rows:
        unescaped_pipes = len(re.findall(r"(?<!\\)\|", row))
        assert unescaped_pipes == 7, row


def test_terminal_report_untrusted_fields_defanged(
    tmp_path: Path,
) -> None:
    """The terminal renderer must not pass ANSI escapes from remote
